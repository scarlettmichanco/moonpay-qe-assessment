...
test('Cant submit unless all required fields are filled', async t => {
  ...
  await t
    .switchToIframe('#checkout-demo')
    .typeText(await checkoutPage.input.email, 'FooBarBaz@', replaceText)
    .click(checkoutPage.submit.submitButton)
    .expect(validator.cardErrors.invalidEmail.visible).ok()
    .typeText(await checkoutPage.input.email, 'FooBarBaz@gmail.com', replaceText)
    .typeText(checkoutPage.input.cardNumber,'4242 4242 4242 000', replaceText)
    .click(checkoutPage.submit.submitButton)
    .expect(validator.cardErrors.incompleteCC.visible).ok()
    .typeText(checkoutPage.input.cardNumber,'4242 4242 4242 4242', replaceText)
    .typeText(checkoutPage.input.cardCvc, '99', replaceText)
    .click(checkoutPage.submit.submitButton)
    .expect(validator.cardErrors.invalidCVC.visible).ok()
    .typeText(checkoutPage.input.cardCvc, '999', replaceText)
    .typeText(checkoutPage.input.cardExpiry, '02/12', replaceText)
    .click(checkoutPage.submit.submitButton)
    .expect(validator.cardErrors.invalidExpiry.visible).ok()
    .typeText(checkoutPage.input.cardExpiry, '02/25', replaceText)
    .typeText(checkoutPage.input.billingName, 'Foo Bar', replaceText)
    .typeText(checkoutPage.input.billingPostalCode, '0211', replaceText)
    .click(checkoutPage.submit.submitButton)
    .expect(validator.cardErrors.invalidZip.visible).ok()
    .typeText(checkoutPage.input.billingPostalCode, '02115', replaceText)
    .click(checkoutPage.submit.submitButton)
